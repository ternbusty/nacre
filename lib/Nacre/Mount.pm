...
sub do_mount_setattr ($dirfd, $path, $flags, $attr_set, $attr_clr, $userns_fd = undef) {
    ...
    my $sz = length($attr) + 0;
    my $ret;
    do {$ret = syscall($nr, $df, $p, $fl, $attr, $sz)} while ($ret == -1 && $! == EINTR);
    return $ret == 0;
}
...
sub do_open_tree ($dirfd, $path, $flags) {
    ...
    my $fl = $flags + 0;
    my $ret;
    do {$ret = syscall($nr, $df, $path, $fl)} while ($ret == -1 && $! == EINTR);
    return $ret;
}
...
sub create_devices ($rootfs, $spec) {
    ...
    for my $dev (@devices) {
        ...
        if (SYS_mknod > 0) {
            my $nr = SYS_mknod + 0;
            do {$ret = syscall($nr, $path, $mode, $dev_num)} while ($ret == -1 && $! == EINTR);
        } else {

            # aarch64: use mknodat with AT_FDCWD (-100)
            my $nr = SYS_mknodat + 0;
            my $dirfd = -100 + 0;
            do {$ret = syscall($nr, $dirfd, $path, $mode, $dev_num)} while ($ret == -1 && $! == EINTR);
        }
        if ($ret == 0) {
