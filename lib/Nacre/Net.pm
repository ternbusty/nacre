...
sub netlink_move_to_ns ($ifindex, $ns_pid) {
    my $msg
        = pack('L S S L L', 40, RTM_SETLINK, NLM_F_REQUEST | NLM_F_ACK, 1, 0)
...
sub netlink_rename ($ifindex, $new_name) {
    ...
    my $msg
        = pack('L S S L L', $total, RTM_SETLINK, NLM_F_REQUEST | NLM_F_ACK, 1, 0)
        . pack('C x S l L L', 0, 0, $ifindex, 0, 0)
...
sub netlink_add_addr ($ifindex, $addr_str) {
    ...
    my $msg
        = pack('L S S L L', $total, RTM_NEWADDR, NLM_F_REQUEST | NLM_F_ACK | NLM_F_CREATE | NLM_F_EXCL, 1, 0)
...
sub netlink_list_addrs ($ifindex) {
    ...
    my $sa = pack('S x2 L L', AF_NETLINK, 0, 0);
    bind($sock, $sa) or fatal("netlink bind: $!");
    my $req = pack('L S S L L', 24, RTM_GETADDR, NLM_F_REQUEST | NLM_F_DUMP, 1, 0) . pack('C C C C l', 0, 0, 0, 0, 0);
